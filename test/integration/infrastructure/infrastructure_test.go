...
func verifyCreation(
	...
	if providerConfig.DualStack != nil && providerConfig.DualStack.Enabled {
		...
		infrastructureIdentifier.ipv6GatewayID = pointer.String(describeIpv6GatewaysOutput.Ipv6Gateways.Ipv6Gateway[0].Ipv6GatewayId)
	}
	...
	for _, vsw := range describeVSwitchesOutput.VSwitches.VSwitch {
		if vsw.VSwitchName == infra.Namespace+"-DUAL_STACK-A-vsw" {
			infrastructureIdentifier.ipv6vswID_A = pointer.String(vsw.VSwitchId)
			Expect(vsw.EnabledIpv6).To(Equal(true))
			ipv6_vsw_count++
		} else if vsw.VSwitchName == infra.Namespace+"-DUAL_STACK-B-vsw" {
			infrastructureIdentifier.ipv6vswID_B = pointer.String(vsw.VSwitchId)
			Expect(vsw.EnabledIpv6).To(Equal(true))
			ipv6_vsw_count++
		} else {
	...
	infrastructureIdentifier.vswitchID = pointer.String(vsw_list[0].VSwitchId)
	if providerConfig.Networks.VPC.CIDR != nil {
